        ...
        for i in range(len(words)):
            ...
            if len(term1) != 0:
                ...
                if doc_id in doc_words:
                    if term1 in doc_words[doc_id]:
                        doc_words[doc_id][term1] += 1
                    else:
                        doc_words[doc_id][term1] = 1
                else:
                    doc_words[doc_id] = {term1: 1}
...
def build_ngram_dictionary_count_dict(ngram_dictionary_count_dict, term='', head=0, tail=0, freq=0):
    """
...
# TO DO - Write this new dict to output file
def write_ngram_dict_output(ngram_dict, ngram_count_dict, output_file_dictionary, output_file_postings):
    ...
    with open(output_file_postings, 'w') as out_postings:
        ...
        for term, doc_id_dict in ngram_dict.iteritems():
            ...
            build_ngram_dictionary_count_dict(ngram_count_dict, term, head, tail, freq)
